...
################################################################################
# Visualisation Layer Resources (S3, Crawler, Glue Data Catalog)
################################################################################
...
resource "aws_glue_crawler" "weather-crawler" {
  database_name = "${aws_glue_catalog_database.weather-catalog.name}"
  name          = "weather-streaming-crawler"
  role          = "${aws_iam_role.glue.name}"

  s3_target {
    path = "s3://${aws_s3_bucket.weather-bucket.bucket}/"
  }

  provisioner "local-exec" {
    command = "aws glue start-crawler --name ${aws_glue_crawler.weather-crawler.name}"
  }
}
